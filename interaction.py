...
class LocalSystem:
    ...
    def __init__(self,
        ...
        if qmesh is None:
            self.q_max = 10*2/self.r_c
            self.q, self.wq = utility.log_mesh(0, self.q_max, nq)
        else:
            self.q, self.wq, self.q_max = qmesh
    ...
    def phase_shifts(self, glo, gnlo):
        ...
        return np.array(
            [scatter.phase_shift(ki, v, self.q, self.wq, self.q_max,
    ...
    def effective_range_expansion(self, glo, gnlo, l, use_c=False):
        ...
        if use_c:
            ...
            return np.array(
                [ki**(2*l)*scatter.kcotdelta(ki, v, self.q, self.wq, self.q_max, 2*const.MU) for ki in self.ks]
            )
    ...
    def k3cotd(self, p, glo, gnlo):    
        ...
        pp = p*p
        print(p)
        scatter_temp = scatter.kcotdelta(p, v, self.q, self.wq, self.q_max, 2*const.MU)
    ...
    def k3cotd_gen(self, ks, glo, gnlo):    
        ...
        v = self.v_tilde + xterm
        return np.array(
            [ki**2*scatter.kcotdelta(ki, v, self.q, self.wq, self.q_max, 2*const.MU) for ki in ks]
        )
    ...
    def k3cotd_gen_fast(self, ks, glo, gnlo):    
        ...
        v = self.v_tilde + xterm
        return np.array(
            [mu2.cc.cscatter.kcotdelta_py(ki, v, self.q, self.wq, self.q_max,
                                           self.ell, 2*const.MU) for ki in ks]
        )
    ...
    def effective_range_parameters(self, glo, gnlo, p0=[-1, -1, 1, 1], use_c=False):
        ...
        '''
        k3cds = self.effective_range_expansion(glo, gnlo, self.ell, use_c=use_c)
...
class NonlocalSystem:
    ...
    def __init__(self,
        ...
        self.l = ell
        if rmesh == None:
            self.r_max = 10*const.BETA4
            self.r_nodes, selfr_weights = utility.log_mesh(0, self.r_max, 2000)
        else:
            self.r_nodes, self.r_weights, self.r_max = rmesh
        ...
        self.v_tilde = utility.ft_matrix_gen(lambda r: potential(r, self.r_c/10),
                self.l, self.l, self.q, self.r_nodes, self.r_weights)
    ...
    def effective_range_expansion(self, glo, gnlo, use_c=False):
        ...
        if use_c:
            ...
            return np.array(
                [ki**(2*self.l)*scatter.kcotdelta(ki, v, self.q, self.wq, self.q_max, 2*const.MU) for ki in self.ks]
            )
    ...
    def k3cotd_gen(self, ks, glo, gnlo):    
        ...
        v = self.v_tilde + xterm
        return np.array(
            [ki**(2*self.l)*scatter.kcotdelta(ki, v, self.q, self.wq, self.q_max, 2*const.MU) for ki in ks]
        )
    ...
    def k3cotd_gen_fast(self, ks, glo, gnlo):    
        ...
        return np.array(
            [mu2.cc.cscatter.kcotdelta_py(ki, v, self.q, self.wq, self.q_max,
                                          self.l, 2*const.MU) for ki in ks]
        )
